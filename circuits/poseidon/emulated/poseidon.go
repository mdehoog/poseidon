...
func HashEx[T emulated.FieldParams](field *emulated.Field[T], inputs []*emulated.Element[T], initialState *emulated.Element[T], nOuts int) []*emulated.Element[T] {
	...
	var f T
	modulus := f.Modulus()
	ca, ok1 := constants.C(modulus, t-2)
	sa, ok2 := constants.S(modulus, t-2)
	ma, ok3 := constants.M(modulus, t-2)
	pa, ok4 := constants.P(modulus, t-2)
	if !(ok1 && ok2 && ok3 && ok4) {
		panic("unsupported field")
	}
	c := ArrayToElement[T](ca)
	s := ArrayToElement[T](sa)
	m := Array2ToElement[T](ma)
	p := Array2ToElement[T](pa)
