...
func HashEx(api frontend.API, inputs []frontend.Variable, initialState frontend.Variable, nOuts int) []frontend.Variable {
	...
	c, ok1 := constants.C(api.Compiler().Field(), t-2)
	s, ok2 := constants.S(api.Compiler().Field(), t-2)
	m, ok3 := constants.M(api.Compiler().Field(), t-2)
	p, ok4 := constants.P(api.Compiler().Field(), t-2)
	if !(ok1 && ok2 && ok3 && ok4) {
